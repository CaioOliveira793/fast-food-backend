...
describe('Cooker entity', () => {
	...
	it('get the name', () => {
		const cookerName = new Name(name.findName());
	...
	it('set the name', () => {
		const cooker = Cooker.new(
			new Name(name.findName()),
			new Email(internet.email()),
			internet.password()
		);

		const newCookerName = new Name(name.findName());
	...
	it('get the email', () => {
		...
		const cooker = Cooker.new(new Name(name.findName()), cookerEmail, internet.password());
